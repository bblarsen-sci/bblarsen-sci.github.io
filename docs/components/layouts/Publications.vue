...
<template>
  <div class="max-w-screen-lg mx-auto">
      ...
      <div id="section-1" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-evenly gap-8 px-8">
        <div v-for="paper in preprints" :key="paper.title" class="card">
          <a :href="paper.link" class="block h-full">
            <div
              class="h-full flex flex-col justify-evenly space-y-2 p-6 bg-white dark:bg-slate-700 rounded-xl shadow-md shadow-slate-500">
              <div class="space-y-4">
    ...
    <div class="pb-10">
      <div class="h-1 bg-slate-400"></div>
      <h2 class="text-2xl leading-8 tracking-tight font-bold py-10 px-8">First author publications</h2>
      <div id="section-2" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 px-8">
        <div v-for="paper in publications" :key="paper.title" class="card">
          <a :href="paper.link" class="block h-full">
            <div
              class="h-full flex flex-col justify-between space-y-2 p-6 bg-white dark:bg-slate-700 rounded-xl shadow-md shadow-slate-500">
              <div class="space-y-4">
                <div class="text-xl leading-8 font-semibold tracking-tight">{{ paper.title }}</div>
                <div class="flex justify-center">
    ...
    <div class="pb-10">
      <div class="h-1 bg-slate-400"></div>
      <h2 class="text-2xl leading-8 tracking-tight font-bold py-10 px-8">Co-authored Publications</h2>
