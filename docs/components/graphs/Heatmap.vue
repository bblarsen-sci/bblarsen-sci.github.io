...
function updateHeatmap(data) {
  ...
  const innerWidth = squareSize * sites.length; // Define the inner width based on the square size and number of sites
  ...
  const xScale = d3.scaleBand() // Define the x scale based on the sites
    .domain(sites)
    .range([0, innerWidth])
    .padding(paddingValue.value);
  ...
  const xAxis = d3.axisBottom(xScale).tickSizeOuter(0); // Add the x-axis, and only display the numbers on x-axis if there are less than 50 sites, otherwise display every 10th number
  if (sites.length <= 50) {
